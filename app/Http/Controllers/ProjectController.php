...
class ProjectController extends Controller
{
    ...
    public function popup($slug, $projectID)
    {
        ...
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        $project = Project::select('projects.*')->join('user_projects', 'projects.id', '=', 'user_projects.project_id')->where('user_projects.user_id', '=', $objUser->id)->where('projects.workspace', '=', $currentWorkspace->id)->where('projects.id', '=', $projectID)->first();
        $user_project = UserProject::get();

        return view('projects.invite', compact('currentWorkspace', 'project', 'user_project'));
    }
        ...
        if ($project) {
            ...
            if ($stage) {
                ...
                if ($objUser->getGuard() == 'client') {
                    ...
                    )->with('success', __('Task Create Successfully!').$google_msg.'');
                } else {
                    return redirect()->route(
                        'projects.task.board',
                        [
                    ...
                    )->with('success', __('Task Create Successfully!').'</br><span class="text-danger">'.$google_msg.'</span>' );
                }
            } else {
    ...
    public function taskShow($slug, $projectID, $taskID)
    {
        ...
        $clientID = '';
        if (isset($objUser) && $objUser->getGuard()  == 'client') {
            $clientID = $objUser->id;
        }
    ...
    // public function googletaskShow($taskID){
    //     $google_calendar = Event::find($taskID);
    //     return view('projects.taskShow', compact('google_calendar'));
    // }
    ...
    public function commentStore(Request $request, $slug, $projectID, $taskID, $clientID = '')
    {
        ...
        if (isset($settings['telegram_taskcom_notificaation']) && $settings['telegram_taskcom_notificaation'] == 1) {
            Utility::send_telegram_msg('New Task Comment' ,$uArr, $user);
    ...
    public function commentDestroyFile(Request $request, $slug, $projectID, $taskID, $fileID)
    {
        ...
        if ($commentFile) {
            ...
            if(\Storage::disk($setting['storage_setting'])->exists('tasks/' . $commentFile->file)){
    ...
    public function subTaskStore(Request $request, $slug, $projectID, $taskID, $clientID = '')
    {
        ...
        $subtask->deleteUrl = route(
            $client_keyword.'subtask.destroy',
            [
                $slug,
                $projectID,
                $subtask->id,
    ...
    public function fileDelete($slug, $id, $file_id)
    {
        ...
        if ($file) {
            ...
            if(\Storage::disk($setting['storage_setting'])->exists('project_files/' . $file->file_path)){
                $file_path = 'project_files/'.$file->file_path;
                $result = Utility::changeStorageLimit(\Auth::user()->id, $file_path); 
                // \File::delete($path);
            }
            $file->delete();
    ...
    public function ajax_tasks($slug, Request $request)
    {
        ...
        foreach ($tasks as $task) {
            ...
            if ($currentWorkspace->permission == 'Owner') {
                ...
                $tmp['action'] = '
                <a href="#" class="action-btn btn-info  btn btn-sm d-inline-flex align-items-center" data-toggle="popover"  title="' . __('Edit Task') . '" data-ajax-popup="true" data-size="lg" data-title="' . __('Edit') . '" data-url="' . route(
                    'tasks.edit',
                    [
                        $currentWorkspace->slug,
                        $task->project_id,
    ...
    public function filterTimesheetTableView(Request $request, $slug)
    {
        ...
        if($project){
            ...
            $objUser1 = $currentWorkspace->owner($currentWorkspace->id);
            $objUser = $objUser1;
        }
        if($objUser->getGuard() != 'client'){
        ...
        if ($project) {
            ...
            if ($task && $selected_dates) {
                ...
                foreach ($period as $key => $dateobj) {
                    $returnHTML .= '<td><div role="button" class="form-control border-dark wid-120" data-ajax-timesheet-popup="true" data-type="create" data-task-id="' . $task->id . '" data-date="' . $dateobj->format('Y-m-d') . '" data-url="' . route('project.timesheet.create', [
    ...
    public function copylink_setting_create($slug , $projectID)
    {
        ...
        // dd($project ,$project->copylinksetting);
