...
class ProjectReportController extends Controller
{
    ...
    public function show(Request $request ,$slug,$id)
    {
     $objUser = Auth::user();
        ...
        if ($project) 
        {
            ...
            $project_status_task = stage::join("tasks", "tasks.status", "=", "stages.id")->where("workspace_id", "=", $currentWorkspace->id)->where('tasks.project_id', '=', $id)->groupBy('name')->selectRaw('count(tasks.id) as count, name')->pluck('count', 'name');
        ...
        foreach($projects as $project) {
            ...
            $tmp['start_date'] = $project->start_date;
             ...
             $tmp['members'] = '<div class="user-group mx-2">';
                $images_path=Utility::get_file('users-avatar/');
                     foreach($project->users as $user){
                        $path =asset($images_path.$user->avatar);
           ...
           $tmp['Progress'] = 
            ...
            if (Auth::user()->getGuard() != 'client') {
                ...
                $tmp['action'] = '
                <a  class="action-btn btn-warning  btn btn-sm d-inline-flex align-items-center" data-toggle="popover"  title="' . __('view Project Report') . '" data-size="lg" data-title="' . __('show') . '" href="' . route(
    ...
    public function ajax_data_report(Request $request ,$slug)
    {
   ...
   public function ajax_tasks_report(Request $request ,$slug,$id)
    {
        $userObj = Auth::user();
        $client_keyword = $userObj->getGuard() == 'client' ? 'client.' : '';
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        ...
        foreach($timesheets_task as $timesheet){
          ...
          $hour_format_number = number_format($logged_hour, 2, '.', '');
            ...
            $tmp['title'] = '<a href="' . route(
                $client_keyword.'projects.task.board', [
                    $currentWorkspace->slug,
                    $task->project_id,
                ]
...
public function export($id)
    ...
    public function uerReport($slug)
    {
        $objUser = Auth::user();
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        ...
        if ($objUser && $currentWorkspace) {
            ...
            return view('users_report.index', compact('currentWorkspace', 'stages', 'users', 'projects'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }

    public function graphShow($slug, $id)
    {
        $objUser = User::find($id); // Retrieve the user using the $id parameter
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        ...
        if ($objUser && $currentWorkspace) {
            ...
            if ($objUser) {
                $projectEachTotalMonth = [];            
                for ($i = 0; $i < 12; $i++) {
                    $firstDayOfMonth = now()->subMonths($i)->firstOfMonth();            
                    $lastDayOfMonth = now()->subMonths($i)->lastOfMonth();
                    $count = $objUser->projects()
                        ->join('projects as p', 'user_projects.project_id', '=', 'p.id')
                        ->where('p.status', 'Finished')
                        ->whereBetween('p.created_at', [$firstDayOfMonth, $lastDayOfMonth])
                        ->count();            
                    $projectEachTotalMonth[$firstDayOfMonth->format('F')] = $count;
                }            
                $objUser->project_each_total_month = $projectEachTotalMonth;
                $objUser->project_each_total_year = $objUser->projects()
                    ->join('projects as p', 'user_projects.project_id', '=', 'p.id')
                    ->where('p.status', 'Finished')
                    ->whereYear('p.created_at', now()->year)
                    ->count();
            }
            return view('users_report.show', compact('currentWorkspace', 'stages', 'objUser', 'projects'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
