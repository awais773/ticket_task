...
class AiTemplateController extends Controller
{
    ...
    public function AiGenerate(Request $request) 
    {
        if ($request->ajax()) 
        {
            ...
            $data       = array();
            $key_data   = DB::table('admin_payment_settings')->where('name','chatgpt_key')->first();
            if($key_data)
            {
            ...
            if( isset($template) && $template->module == 'notification template'){
                $format = $request->format;
                $prompt = $prompt.' '.$format;
            }

            if( isset($template) && $template->module == 'email template'){
                $format = $request->format;
                $prompt = $prompt.' '.$format;
            }
