...
class CashfreeController extends Controller
{
    ...
    public function planPayWithcashfree(Request $request)
    {
        ...
        if ($plan) {
            ...
            if($request->cashfree_payment_frequency == 'annual'){
            ...
            try {
                if (!empty($request->coupon)) {
                            ...
                            if ($assignPlan['is_success'] == true && !empty($plan)) {
                                ...
                                Order::create(
                                    [
                                        ...
                                        'payment_frequency' => $request->cashfree_payment_frequency,
                                        'txn_id' => '',
                                ...
                                $assignPlan = $authuser->assignPlan($plan->id ,$request->cashfree_payment_frequency);
                    ...
                    $orderID = strtoupper(str_replace('.', '', uniqid('', true)));
                    $frequency = $request->cashfree_payment_frequency;
                    ...
                    $data = json_encode([
                        ...
                        "order_meta" => [
                            "return_url" => route('cashfreePayment.success') . '?order_id={order_id}&order_token={order_token}&plan_id=' . $plan->id . '&amount=' . $get_amount . '&coupon=' . $coupon . '&frequency=' . $frequency .''
        ...
        try {
            ...
            if ($respons->order_id && $respons->cf_payment_id != NULL) {
                ...
                if ($info->payment_status == "SUCCESS") {
                    ...
                    $order->payment_frequency = $request->frequency;
                    $order->txn_id = '';
                    $order->receipt = '';
