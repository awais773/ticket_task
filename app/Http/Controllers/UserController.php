 ...
 public function import(Request $request)
    ...
    public function store(Request $request)
    {
        if(Auth::user()->type == 'admin')
        {
            ...
            return redirect()->route('users.index', '')->with('success', __('User Created Successfully'.$msg));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function addPromotionEdit($slug, $id)
    {
        $user             = User::find($id);
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
    ...
    public function update($slug = null, $id = null, Request $request)
    {
        ...
        $objUser->project_each_month = $request->project_each_month;
    ...
    public function addPromotion($slug = null, $id = null, Request $request)
    {
        $currentWorkspace = Utility::getWorkspaceBySlug($slug);
        $workspace_owner = isset($currentWorkspace) ? $currentWorkspace->owner($currentWorkspace->id) : '';
        if($id)
        ...
        $existingSalary = $objUser->employee_salary ?? 0;
        $objUser->employee_salary = $existingSalary + $request->employee_salary;
        $dir = 'users-avatar/';
        ...
        return redirect()->back()->with('success', __('User Promotion Successfully!').((isset($result) && $result!=1) ? '<br> <span class="text-danger">' . $result . '</span>' : ''));
    }
