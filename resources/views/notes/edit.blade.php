...
<form class="" method="post" action="{{ route('notes.update',[$currentWorkspace->slug,$note->id]) }}">
    ...
    <div class="row">
        ...
        <div class="text-end col-12">
            <a href="#" data-size="lg" data-ajax-popup-over="true" class="btn btn-sm btn-primary" data-url="{{ route('generate',['notes']) }}" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Generate with AI') }}" data-title="{{ __('Generate Notes Title & Description') }}">
        ...
        <div class="col-md-12">
            <label for="title" class="col-form-label">{{ __('Building/Department Name') }}</label>
            <input class="form-control" type="text" id="building_name" name="building_name" placeholder="{{ __('Enter Name') }}"  value="{{$note->building_name}}" required>
        </div>
        <div class="col-md-12">
            <label for="title" class="col-form-label">{{ __('Building Number') }}</label>
            <input class="form-control" type="number" id="building_number" name="building_number" placeholder="{{ __('Enter Number') }}" value="{{$note->building_number}}"  required>
        </div>
        <div class="col-md-12">
            <label for="description" class="col-form-label">{{ __('Description') }}</label>
            <textarea rows="3" class="form-control grammer_textarea" id="description" name="text" placeholder="{{ __('Enter Description') }}" required>{{$note->text}}</textarea>
        </div>
        <div class="col-md-12">
