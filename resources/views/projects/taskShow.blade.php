...
@if($currentWorkspace && $task)
            ...
            <div class="col-md-3">
                ...
                @if($users = $task->users())
                    ...
                    <a href="#"  data-toggle="tooltip" data-placement="top" title="{{$user->name}}">
                        <img @if($user->avatar) src="{{asset($logo.$user->avatar)}}" @else avatar="{{$user->name}}" @endif class="rounded-circle mt-1 w-20">
                    </a>                   
                     @endforeach
                @endif
            </div>
    ...
    <div class="tab-content" id="myTabContent">
        <div class="tab-pane fade active show" id="comments-data" role="tabpanel" aria-labelledby="home-tab">
            <form method="post" id="form-comment" data-action="{{route($client_keyword.'comment.store',[$currentWorkspace->slug,$task->project_id,$task->id,$clientID])}}">
        ...
        <div class="tab-pane fade" id="file-data" role="tabpanel" aria-labelledby="profile-tab">
            <div class="form-group m-0">
                <form method="post" id="form-file" enctype="multipart/form-data" data-url="{{ route($client_keyword.'comment.store.file',[$currentWorkspace->slug,$task->project_id,$task->id,$clientID]) }}">
                    ...
                    <div class="card pb-0 mb-1 shadow-none border">
                        <div class="card-body p-3">
                            <div class="row align-items-center">
                                <div class="col-auto">
                                    <div class="avatar-sm">
                                        <span class="avatar-title rounded text-uppercase">
                                              <img src="{{asset($logo_tasks.$file->file)}}" class="preview_img_size" id=""/>
                                         <!--    {{$file->extension}} -->
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="col-auto">
                                    <!-- Button -->
                                    <a /sho href="{{$logo_tasks.$file->file}}" class="action-btn btn-primary  btn btn-sm d-inline-flex align-items-center">
                                        <i class="ti ti-download" data-toggle="popover" title="{{ __('Download') }}"></i>
                                    </a>

                                    <a class="action-btn btn-secondary  btn btn-sm d-inline-flex align-items-center" href="{{$logo_tasks.$file->file}}" target="_blank"  >
                                        <i class="ti ti-crosshair text-white" data-toggle="popover" title="{{ __('Preview') }}"></i>
                                    </a>

                                    @auth('web')
                                        <a id="123456" href="#" class="action-btn btn-danger  btn btn-sm d-inline-flex align-items-center delete-comment-file" data-url="{{route('comment.destroy.file',[$currentWorkspace->slug,$task->project_id,$task->id,$file->id])}}">
        ...
        <div class="tab-pane fade mt-3" id="sub-task-data" role="tabpanel" aria-labelledby="contact-tab">
            ...
            <form method="post" id="form-subtask" class="collapse" data-action="{{route($client_keyword.'subtask.store',[$currentWorkspace->slug,$task->project_id,$task->id,$clientID])}}">
                @csrf
                <div class="card-body">
                    <div class="row">
                        <div class="col-6">
                ...
                @foreach($task->sub_tasks as $subTask)
                    <li class="list-group-item py-3">
                        <div class="form-check form-switch d-inline-block">
                            <input type="checkbox" class="form-check-input" name="option" id="option{{ $subTask->id }}" @if($subTask->status) checked @endif data-url="{{route($client_keyword.'subtask.update',[$currentWorkspace->slug,$task->project_id,$subTask->id])}}">
