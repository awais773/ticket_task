@if($project && $currentWorkspace && $bug)
    ...
    <form class="" method="post" action="@auth('web'){{ route('projects.bug.report.update',[$currentWorkspace->slug,$project->id,$bug->id]) }}@elseauth{{ route('client.projects.bug.report.update',[$currentWorkspace->slug,$project->id,$bug->id]) }}@endauth">
        @csrf
         <div class="modal-body">
        <div class="row">
            @if ($currentWorkspace->is_chagpt_enable())
            <div class="text-end col-12">
                <a href="#" data-size="lg" data-ajax-popup-over="true" class="btn btn-sm btn-primary" data-url="{{ route('generate',['bug']) }}" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Generate with AI') }}" data-title="{{ __('Generate Bug Title & Description') }}">
            ...
            <div class="form-group col-md-12 mb-0">
                ...
                <textarea rows="3" class="form-control" id="task-description"  name="description">{{$bug->description}}</textarea>
            </div>
        </div>
    </div>
