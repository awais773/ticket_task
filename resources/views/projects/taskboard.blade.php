                                ...
                                <div class="card" id="{{$task->id}}">
                                   <!--  <img class="img-fluid card-img-top" src=""
                                    ...
                                    <div class="card-header border-0 pb-0 position-relative">
                                        ...
                                        <div style="padding: 30px 2px;"> <a href="#" data-size="lg"  data-url="{{route($client_keyword.'tasks.show',[$currentWorkspace->slug,$task->project_id,$task->id])}}" data-ajax-popup="true" data-title="{{__('Task Detail')}}" class="h6 task-title"><h5>{{$task->title}}</h5></a></div>
                                                ...
                                                @foreach($users as $key => $user)
                                                    @if($key < 3)
                                                        <a href="#" class="img_group"   data-toggle="tooltip" data-placement="top" title="{{$user->name}}">
                                                            <img alt="image" data-toggle="tooltip" data-original-title="{{$user->name}}" @if($user->avatar) src="{{asset($logo.$user->avatar)}}" @else avatar="{{ $user->name }}"@endif class="rounded-circle mt-1 w-20" >
                                                        </a>
                                                    @endif
                                                @endforeach
                                                @if(count($users) > 3)
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if(data.result == 1){
                            ...
                            show_toastr('{{__('Success')}}', '{{ __("File not Uploded!")}}', 'success');
                        }
                        var delLink = '';
