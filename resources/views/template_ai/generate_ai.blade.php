...
<form action="" id="myForm">
    ...
    <div class="row">
            ...
            <div class="form-group">    
                ...
                <select name="language" class="form-select" id="language">
                    @foreach (App\Models\Utility::flagOfCountry() as $key => $lang)
                        <option value="{{ $key }}" {{ $currantLang == $key ? 'selected' : '' }}>{{ ucfirst($lang) }}</option>
                    @endforeach   
                </select>
            </div>
        </div>
        ...
        <div class="col-6">
            <div class="form-group">
                ...
                <input type="hidden" name="format" value="{{ $content_format }}" id="format">
            </div>
        </div>
    ...
    <div class="response row col-sm-12" >
            <a class="btn btn-primary btn-sm col-md-2 col-12 my-2 mx-2 " href="#!" id="generate">{{ __('Generate') }}</a>
            <a href="#!" onclick="copyText()" class="btn btn-primary btn-sm col-12 col-md-2 my-2 mx-2 float-end "><i class="ti ti-copy"></i>{{ __('Copy Text') }}</a>
            <a href="#!" onclick="copySelectedText()" class="btn btn-primary btn-sm col-12 col-md-3 my-2 mx-2 float-end me-2"><i class="ti ti-copy"></i>{{ __('Copy Selected Text') }}</a>
    </div>
</form>
...
<script>
    function copyText() {
        ...
        if(input>0){
            ...
            if($('textarea[name='+selected+']').hasClass('pc-tinymce-2')){
                ...
                tinymce.get('summernote').setContent(copied);
            }
    ...
    function copySelectedText() {
        ...
        if(input>0){
            ...
            if($('textarea[name='+selected+']').hasClass('pc-tinymce-2')){
                tinymce.get('summernote').setContent(selectedText)
                }
                else if($('textarea[name='+selected+']').hasClass('summernote-simple'))
                {
                $("#summernote-simple").summernote('code', copied);
