                                ...
                                <div class="d-grid">
                                    ...
                                    @endif
                                        ...
                                        <div class="d-grid col-12 mt-3">
                                                <a href="{{route('login', $lang)}}" type="button" class="btn btn-primary btn-block mt-2" style="color:#fff">{{ __('User Login') }}</a>
                                        </div>
                                            ...
                                            <a href="#" class="  btn-primary  ">
                                                <select name="language" id="language" class=" btn-primary btn " onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                                                    @foreach(App\Models\Utility::languages() as $language)
                                                        <option class="login_lang" @if($lang == $language) selected @endif value="{{ route('client.login',$language) }}">{{ucfirst( \App\Models\Utility::getlang_fullname($language))}}</option>
                                                    @endforeach
                                                </select>
                                            </a>
                                            @endsection
