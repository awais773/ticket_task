...
@section('action-button')
    ...
    @auth('web')
        @if ($currentWorkspace->creater->id == Auth::user()->id)
            <a href="#" class="btn btn-sm mx-1  btn-primary" data-size="lg" data-bs-toggle="tooltip"
                data-bs-original-title="{{ __('Edit') }}" data-ajax-popup="true" data-title="{{ __('Edit Invoice') }}"
                data-url="{{ route('invoices.edit', [$currentWorkspace->slug, $invoice->id]) }}">
                <i class="ti ti-edit"></i>
            </a>
            <a href="#" class="btn btn-sm  mx-1 btn-primary cp_link "
                data-link="{{ route('pay.invoice', [$currentWorkspace->slug, \Illuminate\Support\Facades\Crypt::encrypt($invoice->id)]) }}"
                data-toggle="tooltip" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Copy Invoice') }}"
        ...
        @if ($currentWorkspace->creater->id == Auth::user()->id)
            @if ($invoice->getDueAmounts($invoice->id) > 0)
                <a href="#" data-toggle="modal" data-target="#addPaymentModal" data-bs-toggle="tooltip"
                    data-bs-original-title="{{ __('Add Payment') }}" class="btn btn-sm  mx-1  btn-primary" type="button">
...
@section('content')
        ...
        <div class="card" id="printTable">
            ...
            <div class="card-body">
                                        ...
                                        <tr>
                                            ...
                                            @elseif($invoice->status == 'partialy paid')
                                                <td style="padding-bottom: 0px  !important; font-size: 15px !important;">
                                                    <b>{{ __('Status') }} :</b>
                                                    <span
                                                        class="p-2 px-3 rounded badge bg-warning">{{ __('Partialy Paid') }}</span>
                                                </td>
                                    ...
                                    @foreach ($invoice->items as $key => $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->task ? $item->task->title : '' }}-
                                                <b>{{ $item->task ? $item->task->project->name : '' }}</b>
                                            </td>
                            ...
                            <table class="table table-responsive invoice-table ">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-primary m-r-10 px-2">
                                            {{ $currentWorkspace->priceFormat($invoice->getDueAmounts($invoice->id)) }}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                @php
                    ...
                    $plan = App\Models\Plan::find($workspace_owner->plan);
                @endphp
                ...
                @if ($payments)
                    <div class="row">
                        <div class="col-sm-12">
                            ...
                            @if($plan->storage_limit <= $workspace_owner->storage_limit && $plan->storage_limit != -1)
                                <h6 class="px-2 text-danger">{{ __('Note: Your plan storage limit is over , so you can not see clients uploaded payment receipt.') }}</h6>
                            @endif
                            <div class="table-responsive mt-3">
                                <table class="table  invoice-detail-table">
                                    <thead>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if (!empty($payment->receipt))
                                                        @if($plan->storage_limit > $workspace_owner->storage_limit && $plan->storage_limit != -1)
                                                            <a href="{{ \App\Models\Utility::get_file('uploads/invoice_receipt/' .$payment->receipt) }}" target="_blank"
                                                ...
                                                @if(Auth::user()->type == 'user'&& $currentWorkspace->creater->id == Auth::user()->id)    
                                                    <td>
                                                        @if($payment->payment_type == 'Bank Transfer' && $payment->payment_status == 'pending')
                                                            <a href="#" class="action-btn btn-warning  btn btn-sm d-inline-flex align-items-center" data-url="{{ route('invoice.status.show', [$currentWorkspace->slug,$payment->id ]) }}" data-toggle="tooltip" title="{{__('Edit')}}"     data-size="lg" data-ajax-popup="true" data-title="{{__('Payment Status')}}">
                                                                <i class="ti ti-caret-right"></i>
                                                            </a>
                                                        @endif
            
                                                        <a href="#" class="action-btn btn-danger  btn btn-sm d-inline-flex align-items-center bs-pass-para" data-confirm="{{__('Are You Sure?')}}" data-text="{{__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="delete-form-{{$payment->id}}" data-toggle="tooltip" title="{{__('Delete')}}"            >
                                                            <i class="ti ti-trash"></i>
                                                        </a>
                                                        {!! Form::open(['method' => 'DELETE', 'route' => ['invoice.payments.destroy', $payment->id],'id'=>'delete-form-'.$payment->id]) !!}
                                                        {!! Form::close() !!}
                                                    </td>
                                                @endif
                                            </tr>
    ...
    @if (auth('web') && $invoice->getDueAmounts($invoice->id) > 0)
        <!-- Modal -->
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="form-icon-user">
                                        ...
                                        <input class="form-control currency_input" type="number" id="amount"
                                            name="amount" value="{{ $invoice->getDueAmounts($invoice->id) }}"
                                            min="0" step="0.01"
                        ...
                        <div class="modal-body">
                            <div class="card-box">
                                    ...
                                    <ul class="nav nav-tabs bordar_styless py-3 my-2">
                                        ...
                                        @if ($currentWorkspace->is_stripe_enabled == 1)
                                            <li class="pt-3">
                                                ...
                                                aria-controls="paystack" aria-selected="false"
                                                    >{{ __('Stripe') }}</a>
                                            </li>
                                        @endif
                                        ...
                                        @if (isset($paymentSetting['is_payfast_enabled']) && $paymentSetting['is_payfast_enabled'] == 'on')
                                            <li class="pt-3" >
                                                <a data-toggle="tab" href="#payfast-payment" onclick="return get_payfast_status(amount = 0,coupon = null);" role="tab"
                                ...
                                <div class="tab-content mt-3">
                                    ...
                                    @if ($currentWorkspace->is_paypal_enabled == 1)
                                        <div class="tab-pane fade 
                                        {{-- {{ $currentWorkspace->is_stripe_enabled == 0 && $currentWorkspace->is_paypal_enabled == 1 ? 'show active' : '' }} --}}
                                        "
...
@endsection
@auth('client')
    @if (($invoice->getDueAmounts($invoice->id) > 0 && $currentWorkspace->is_stripe_enabled == 1) ||
        $currentWorkspace->is_paypal_enabled == 1 ||
        ...
        @push('scripts')
            ...
            {{-- @if ($paymentSetting['is_payfast_enabled'] == 'on' && !empty($paymentSetting['payfast_merchant_id']) && !empty($paymentSetting['payfast_merchant_key']))
                ...
                function get_payfast_status(amount,coupon){
                    ...
                    $.ajax({
                        url: '{{ route('client.invoice.pay.with.payfast') }}',
                        method: 'POST',
                        data : {
