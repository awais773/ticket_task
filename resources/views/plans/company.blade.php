        ...
        <div class="col-xl-12 col-lg-12 col-md-12 d-flex align-items-center justify-content-end">
            <div class="text-sm-right status-filter">
                <div class="btn-group  nav nav-tabs">
                    <a data-toggle="tab" href="#monthly-biling"
                        class="btn_tab btn btn-light bg-primary active  text-white">{{ __('Monthly Biling') }}</a>
                    <a data-toggle="tab" class="btn_tab btn btn-light bg-primary  text-white"
                        href="#annual-billing">{{ __('Annual Billing') }}</a>
                </div>
            </div>
        </div>
    </div>
                                ...
                                <div class="col-xl-3 col-lg-4 col-md-6">
                                    <div class="card price-card price-1 wow animate__fadeInUp" data-wow-delay="0.4s"
                                        ...
                                        <div class="card-body">
                                            ...
                                            @if (\Auth::user()->type == 'user' && \Auth::user()->plan == $plan->id && $userFrequency == 'monthly')
                                            ...
                                            @endif
                                            <span
                                                class="mb-4 f-w-600 p-price">{{ env('CURRENCY_SYMBOL') ? env('CURRENCY_SYMBOL') : '$' }}{{ $plan->monthly_price }}<small
                                                    class="text-sm">/{{ __('Per month') }}</small></span>
                                            <p class="mb-0">
                                                {{ $plan->description }}
                                            </p>
                                            ...
                                            @if (Auth::user()->type != 'admin')
                                                <div class="d-grid text-center">
                                                    ...
                                                    @if (\Auth::user()->plan == $plan->id && Auth::user()->is_trial_done == 1)
                                                        ...
                                                        @endif
                                                    @elseif(
                                                        \Auth::user()->plan == $plan->id && $userFrequency == 'monthly' &&
                                                            (empty(\Auth::user()->plan_expire_date) || date('Y-m-d') < \Auth::user()->plan_expire_date) || $plan->id == 1)
                                                        <p class="">
                                                    ...
                                                    @elseif(
                                                       ...
                                                       {{-- @dd($userFrequency) --}}
                                                        <div class="row">
                                                            @if ($plan->id != 1 && \Auth::user()->plan != $plan->id || $userFrequency != 'monthly')
                                                                <div class="col-8">
                                                                    <button
                                                                        class="btn mb-3 btn-primary d-flex justify-content-center align-items-center">
                                                                        <a href="{{ route('payment', ['monthly', \Illuminate\Support\Facades\Crypt::encrypt($plan->id)]) }}"
                                                                ...
                                                                <div class="col-4">
                                                                    ...
                                                                    @elseif(isset($planRequest) && $planRequest->duration == 'monthly')
                                                                        <ul class="list-unstyled">
                                                                            <li>
                            ...
                            @endphp
                                ...
                                ">
                                        <div class="card-body">
                                            ...
                                            <span
                                                class="mb-4 f-w-600 p-price">{{ env('CURRENCY_SYMBOL') ? env('CURRENCY_SYMBOL') : '$' }}{{ $plan->annual_price }}<small
                                                    class="text-sm">/{{ __('Per Year') }}</small></span>
                                            <p class="mb-0">
                                                {{ $plan->description }}
                                            </p>
                                            <ul class="list-unstyled my-5">
                                            ...
                                            @if (Auth::user()->type != 'admin')
                                                <div class="d-grid text-center">
                                                    ...
                                                    @if (\Auth::user()->plan == $plan->id && Auth::user()->is_trial_done == 1)
                                                                    ...
                                                                    <div class="col-8">
                                                                        <button
                                                                            ...
                                                                            <a href="{{ route('payment', ['annual', \Illuminate\Support\Facades\Crypt::encrypt($plan->id)]) }}"
                                                                                ...
                                                                                class="text-white">
                                                                                <i
                                                                                    class="ti ti-shopping-cart px-2 text-white"></i>{{ __('Subscribe') }}</a>
                                                                        </button>
                                                                    </div>
                                                                    <div class="col-4">
                                                                        @if (\Auth::user()->requested_plan != $plan->id )
                                                        ...
                                                        @endif
                                                    @elseif(
                                                        \Auth::user()->plan == $plan->id &&  $userFrequency == 'annual' &&
                                                            (empty(\Auth::user()->plan_expire_date) || date('Y-m-d') < \Auth::user()->plan_expire_date) || $plan->id == 1)
                                                        <p class="">
                                                            @if (!empty(\Auth::user()->plan_expire_date) && $plan->id != 1)
                                                                {{ __('Plan Expires on ') }}
